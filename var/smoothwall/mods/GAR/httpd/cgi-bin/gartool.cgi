...
my ${conf_file} = ${GAR_Home_dir} . '/etc/gartool.conf';
...
sub do_sidmap
{
    ...
    open(MY_INPUT,"$GAR_Home_dir}/usr/bin/create-sidmap.pl ${rules_loc}|");
    while(<MY_INPUT>) {
	my ${line} = $_;
	chomp(${line});
...
print <<END;
	...
	<table width='100%'>
	    <tr>
		<td align='right'><B>${version} for ${appID} w/ Snort ${SnortDisplayVersion}</B></td>
	    </tr>
	</table>
    </FORM>
END
