       ...
       foreach $_ (@vars)
       {
               ...
               if ($var eq 'ip')
               {
	               print "IP\n";
                       ...
                       {
                               open (FILE,">>$filename") or die '1 - GARipblock.cgi unable to open config file.';
                               flock FILE, 2;
...
if (($cgiparams{'ACTION'} eq $tr{'add'}) || ($cgiparams{'ACTION'} eq $tr{'update'}))
{
       ...
       open (FILE, $filename) or die '2 - GARipblock.cgi unable to open config file.';
       @current = <FILE>;
       close (FILE);
       ...
       unless ($errormessage)
       {
	    ...
	    # Mod for new edit/update procedure
            ...
            {
                    open (FILE,">>$filename") or die '3 - GARipblock.cgi unable to open config file.';
                    flock FILE, 2;
                    ...
                    open (FILE,">$filename") or die '4 - GARipblock.cgi unable to open config file.';
                    flock FILE, 2;
                    $id = 0;
...
{
       open (FILE, "$filename") or die '5 - GARipblock.cgi unable to open config file.';
       @current = <FILE>;
       close (FILE);
       ...
       unless ($errormessage)
       {
               open (FILE, ">$filename") or die '6 - GARipblock.cgi unable to open config file.';
               flock FILE, 2;
	       # go ahead and open the GAR unblock file
		open (UNBLOCK, ">>$ipunblockfilename") or die '7 - GARipblock.cgi unable to open unblock file.';
            	flock UNBLOCK, 2;
               $id = 0;
               foreach $line (@current) {
                       $id++;
...
open (FILE, "$filename") or die '8 - GARipblock.cgi unable to open config file.';
@current = <FILE>;
close (FILE);
