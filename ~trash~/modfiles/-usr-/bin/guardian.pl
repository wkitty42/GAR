#!/usr/bin/perl
# Guardian Active Response - IP Blocker for Snort Alerts
#
my $progname = "guardian.pl";
my $version = "v4.0.0.0-SW3-RC1";
my $partial_secs = 100;                         # how many parts to divide a second into
my $loop_secs = 5;                              # how many seconds to pass before doing housework chores
#
# This version is an extensive rewrite and reorganization based on v1.85. This is being done specifically
# for Smoothwall3 by wkitty42. Thanks to s-t-p for porting NetWhiz's SWE2 version to SWE3 and to NetWhiz
# for doing the work to make this available and operational on SW machines.
# Some code has stayed the same. Numerous new routines have been added. Some existing code has been
# split into seperate routines. A lot of code has been massaged a bit for compatibility with the new
# features and abilities. And finally, some code has simply been removed.
#
# Features and fixes:
#
# v2.0-SW3
#
#  * ipblock comment field contains block date and expiration date
#  * ipblock comment field updated with update date and new expiration date
#    and snort alert that caused the update.
#  * expiration date is only updated if it is more distant in the future than
...
#  * we now monitor the conf file for updates and reprocess it when needed.
#  * we now monitor the ignore file for updates and reprocess it when needed.
#  * we now monitor the target file for updates and reprocess it when needed.
#  * we now monitor the interface for IP changes and adjust accordingly.
#  * IgnoreSIDs config verb - SIDs can now be ignored. Actually, this was already available
#    in older versions but there was no documentation on how to use them. In this rewrite,
#    we simplify and expand on their use. SIDs to ignore require the GID in addition to the
#    SID (ie: 126:2 to ignore encrypted telnet traffic alerts). Put all SIDs to be ignored
#    on one IgnoreSIDs line separated by commas.
#    (ie: IgnoreSIDs            125:7,126:2)
#  * IgnoreGIDs config verb - GIDs (Generator IDs) can now be ignored. For instance, if you
#    want to ignore all SIDs generated by the FTP decoder, you'd list 125 on the IgnoreGIDs
#    config verb line. See the snort generators or gen-msg.map files for the list of
#    generators that snort uses. Put all GIDs to be ignored on one line separated by commas.
#    (ie: IgnoreGIDs            125,126)
#  * fixed gateway address definition for ppp connections. ppp connections should leave the
#    HostGatewayByte undefined in the config file. if the connection is a ppp connection,
#    the gateway will be determined when the host ip address is read from ifconfig.
#  * fixed "bounce" on startup - preload conf, ignore and target file modification timestamps.
#  * new command line option -v displays guardian.pl version
#  * new command line option -k sends terminate signal to running guardian.pl
#  * fixed a bug in the way that the snort alert file's multiline entries are processed
#
# v2.1-SW3
#
#  * added save and reload of existing blocked IPs
#
# v2.2-SW3
#
#  * added dupe checking to saved file reload procedure
#  * fixed bug where loading saved IPs would overwrite existing ipblock file
#
# v2.3-SW3
#
#  * fixed bug where forced logging rule was missing
#
# v3.0-SW3
#
#  * fix unblock file permissions 
#    MAKE SURE THAT INSTALL SCRIPT SETS RIGHT PERMS ON ${swroot}/guardian/ !!
#    they should be 755 nobody:nobody
#  * revert logging to SWE3 *nix style for compatibility and GUI log viewer implementation.
#  * added guardian.log viewer to logs tab.
#      click on the source address for a DNS lookup
#      /httpd/cgi-bin/logs.cgi/guardian.dat (new)
#  * do not attempt to remove an IP listed in unblock unless it is in our block hash.
#  * implemented per source tracking of blocked addresses.
#      this involves a major rewrite of several parts of the code to facilitate
#      recording the attack paper trail in separate data files instead of storing
#      them in the ipblock file where they are/were easier maintained. this adds
#      a rather large measure of complexity to GAR which requires that existing
#      GAR maintained blocks be removed and the entire process started all over.
#      this really isn't all that bad as it should not take all that long for
#      GAR to start recording attacks in the new format ;)
#         /httpd/html/ui/js/script.js       (modified)
#         /httpd/cgi-bin/gartracker.cgi    (new)
#  * fix bug where unblocking LOGged entries was not deleting the ipchains LOG rule
#  * clean up and consolidate tracking strings and their upkeep
#  * implement CIDR recognition for netblocks in guardian.ignore and do nothing other
#      than write a Brief and/or Trivial log message
#      this and the other CIDR routines require the use of the Net::CIDR perl module.
#      this module is /usr/lib/perl5/5.8.8/Net/CIDR.pm
#  * implement recogniton of attacks from manually entered CIDR netblocks in the
#      ipblock file and do nothing other than write a Brief and/or Trivial log message
#  * implement recogniton of attacks from manually entered IPs in the ipblock file and
#      do nothing other than write a Brief and/or Trivial log message
#  * implement checking and cleanup of extraneous tracker files left over from GUI management ops
#  * speed up processing loop to try to catch multiple attacks happening within 1 second
#
# v3.0.1-SW3
#
#  * correct "Updating Tracker File" logging level. should have been 4, not 5
#  * add missing "Updating Tracker File" log level 5
#
# v3.0.2-SW3
#
#  * added last violation time/date and rule to comment field
#  * ensure that comment field text is no more than 255 characters long.
#    this allows for at least 80 displayable characters plus the javascript
#    code for the GARTracker line with up to 3 digits per quad in the
#    dotted decimal IP
#  * cleaned up and clarified more of the logging messages
#
# v3.0.3-SW3
# bugfix - correct PID file assignment :?
# bugfix - the reloading of the saved entries was not properly writing the LOG rule when necessary
# feature - log prefix for forced logging entries...
# bugfix - run smoothcom setipblock to ensure that the table is flushed
#
# TODO:
# support for command line options -a -d and -i...
...
my @msgsymbols = (" ","!","\?","*","+","-","\$","#",":","\%");
my $ip_block_file_name = "${swroot}/ipblock/config";
my $ip_tracker_dir = "${swroot}/guardian/";           # the source IP must be appended to this for tracking
my $saved_ip_block_file_name = "${swroot}/guardian/saved";
my $ip_unblock_file_name = "${swroot}/guardian/unblock";
...
if (defined $opt_h) {
    ...
    print " -v  display GAR version\n";
    print " -k  kill/terminate currently running GAR\n";
...
if (defined $opt_k) {
    if (! -e $pid_file) { die "OPT_K: PID file, $pid_file, not found. Guardian.pl is not running.\n"; }
...
if ( -e $pid_file) { die "PID file, $pid_file, found. Guardian.pl is already running.\n"; }
...
sub build_manual_hashes {
	...
	while (<BLOCKFILE>) {
	    chomp;
	    my @temp = split(/\,/);
	    if ($temp[5] eq 'off') {       # if the entry is NOT managed by guardian
...
sub check_unblockfile {
	...
	open (UNBLOCK, "$ip_unblock_file_name") or die 'CHECK_UNBLOCKFILE: Unable to open ipblock unblock file for reading.';
	flock UNBLOCK, 2;
	my @current = <UNBLOCK>;
	close (UNBLOCK);
	# Open file for writing, lock it, remove each entry from Guardian IF it is in the hash
	open (UNBLOCK, ">$ip_unblock_file_name") or die 'CHECK_UNBLOCKFILE: Unable to open ipblock unblock file for updating.';
	flock UNBLOCK, 2;
	my $line;
	foreach $line (@current) {
	    ...
	    } else {                             # the entry does not exist in 
...
sub check_file_changed {
	...
	if ($ctime > $prev_ipblock_ctime) {      # If the modified timestamp is newer then last we checked,
	    &write_log (2,"ipblock file changed.");
	    $prev_ipblock_ctime = $ctime;
...
#    my $msg = @_[0];
	...
	open (BLOCKFILE, ">>$ip_block_file_name") or die 'UPDATE_IPBLOCK(add): Unable to open ipblock config file for addition.';
	flock BLOCKFILE, 2;
	print BLOCKFILE "$themsg\n";
	close (BLOCKFILE);
    } elsif ($methodtype eq "update") {
	&write_log (5,"Updating entry in ipblock file");
	open (BLOCKFILE, "$ip_block_file_name") or die 'UPDATE_IPBLOCK(update): Unable to open ipblock config file for reading.';
        my @current = <BLOCKFILE>;
	close (BLOCKFILE);
	my $line;
	open (BLOCKFILE, ">$ip_block_file_name") or die 'UPDATE_IPBLOCK(update): Unable to open ipblock config file for updating.';
	flock BLOCKFILE, 2;
	foreach $line (@current) {
	    chomp($line);
		...
		# source matches and Guardian Added flag is 'on', so write the line with the updated comment.
		$line = "$temp[0],$temp[1],$temp[2],$temp[3],$themsg,$temp[5]";
		&write_log (9,"new line: $line");
	    } else {
		# source does not matche and/or Guardian Added flag is 'off', so keep existing line.
...
sub ipchain {
	...
	if ($ForceLogging eq 'yes') {
	    &update_ipblock_file ("add", "$source", "$source,on,DROP,on,\<a href=\"javascript:displayGAR_Tracker('$source');\"\>BLOCKED by Guardian\</a\> ** $blocktimestr ** $type,on");
	    &write_log (5,"Adding $source\'s ipblock LOG rule.");
	    system ("/sbin/iptables -A ipblock -s $source -j LOG --log-prefix 'GAR DROP: '");
	    &write_log (5,"Adding $source\'s ipblock DROP rule.");
	    system ("/sbin/iptables -A ipblock -s $source -j DROP");
	} else {
	    &update_ipblock_file ("add", "$source", "$source,off,DROP,on,\<a href=\"javascript:displayGAR_Tracker('$source');\"\>BLOCKED by Guardian\</a\> ** $blocktimestr ** $type,on");
	    &write_log (5,"Adding $source\'s ipblock DROP rule.");
	    system ("/sbin/iptables -A ipblock -s $source -j DROP");
	}
	...
	if ($block_hash{$source} <= $kicktime) {
	    ...
	    &update_ipblock_file ("update", "$source", "\<a href=\"javascript:displayGAR_Tracker('$source');\"\>BLOCKED by Guardian\</a\> ** $blocktimestr ** $type");
...
sub load_saved {
	...
	my $line;
	open (SAVEDFILE, "$saved_ip_block_file_name") or die 'LOAD_SAVED: Unable to open ipblock save file.';
	my @current = <SAVEDFILE>;
	close (SAVEDFILE);
	open (BLOCKFILE, ">>$ip_block_file_name") or die 'LOAD_SAVED: Unable to open ipblock file.';
	flock BLOCKFILE, 2;
	foreach $line (@current) {
	    chomp($line);
		...
		system ("/sbin/iptables -A ipblock -s $t_line[0] -j DROP");  # add the ip to the DROP table
...
sub load_config {
 if ($opt_c eq "") {$opt_c = "/etc/guardian.conf";}
 if (! -e $opt_c) {die "LOAD_CONFIG: Need a configuration file! Please use to the -c option to name a configuration file\n";}
...
sub validate_config {
 if ($logfile eq "") {
  print "Warning! LogFile is undefined. Assuming /var/log/guardian.log.\n";
 ...
 &write_log (0,"----------------------------------------");
 &write_log (0,"$progname $version initializing...");
 if ($interface eq "") {
...
sub call_unblock {
    ...
    if ($GUI_unblock eq "FALSE") {
        open (BLOCKFILE, "$ip_block_file_name") or die 'CALL_UNBLOCK: Unable to open ipblock config file for reading.';
        my @current = <BLOCKFILE>;
        close (BLOCKFILE);
        my $line;
        open (BLOCKFILE, ">$ip_block_file_name") or die 'CALL_UNBLOCK: Unable to open ipblock config file for updating.';
        flock BLOCKFILE, 2;
        foreach $line (@current) {
	    chomp($line);
	    my @temp = split(/\,/,$line);
	    if ($temp[0] eq $source) {
		if ($temp[5] eq 'on') {      # "Guardian Added" flag is 'on' and source matches, so remove entries
		    # remove the DROP rule
		    &write_log (5,"Removing $source\'s ipblock DROP rule");
		    system ("/sbin/iptables -D ipblock -s $source -j DROP");
		    ...
		    &write_log (2,"$source not found in internal hash???");
		}
	    } else {
    		# Add IP to block file since it does not match unblock IP or the "Guardian Added" flag is 'off'
    		print BLOCKFILE "$line\n";
	    }
	}
	close (BLOCKFILE);
	...
	# the ipblock GUI page has indicated change in a guardian managed rule and requested
...
sub clean_up_and_exit {
	...
	open (IPBLOCK_FILE, "$ip_block_file_name") or die 'CLEAN_UP_AND_EXIT: Unable to open ipblock config file for reading.';
	my @current = <IPBLOCK_FILE>;
	close (IPBLOCK_FILE);
	my $line;
	open (IPSAVE_FILE, ">$saved_ip_block_file_name") or die 'CLEAN_UP_AND_EXIT: Unable to open ipblock save file for writing.';
	flock IPSAVE_FILE, 2;
	my $total = @current;
	...
	foreach $line (@current) {
	    chomp($line);
	    my @temp = split(/\,/,$line);
	    if ($temp[5] eq 'on') {                           # "Guardian Added" flag is 'on'
	...
	# now let's hunt down extraneous tracker files possibly left behind when entries were
	# modified in the GUI page. do this before cleaning out the active block hash so we
	# have something to compare with.
	opendir(DIR, $ip_tracker_dir) || die "CLEAN_UP_AND_EXIT: Can't opendir $ip_tracker_dir: $!";
