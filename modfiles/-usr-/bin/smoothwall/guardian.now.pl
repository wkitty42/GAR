...
#         /httpd/cgi-bin/gar-tracker.cgi    (new)
...
#    if ($counter == 5) {                           # We do these chores every 5 seconds
...
sub create_tracker_file {
    ...
    &write_log (4,"Creating tracker file: $trackedip");
...
sub update_tracker_file {
    my ($trackedip, $msg) = @_;
    my $ip_tracker_file_name = "$ip_tracker_dir$trackedip";
    &write_log (5,"Updating tracker file: $trackedip");
...
sub update_ipblock_file {
    my $msg = @_[0];
    &write_log (5,"Adding new entry to ipblock file: $ip_block_file_name");
    open (BLOCKFILE, ">>$ip_block_file_name") or die 'UPDATE_IPBLOCK: Unable to open ipblock config file.';
    flock BLOCKFILE, 2;
    print BLOCKFILE "$msg\n";
    close (BLOCKFILE);
...
sub ipchain {
	...
	if ($ForceLogging eq 'yes') {
	    &update_ipblock_file ("$source,on,DROP,on,\<a href=\"javascript:displayGAR_Tracker('$source');\"\>BLOCKED by Guardian\</a\>,on");
	    &write_log (5,"Adding iptables LOG rule.");
	    system ("/sbin/iptables -A ipblock -s $source -j LOG");
	    &write_log (5,"Adding iptables DROP rule.");
	    system ("/sbin/iptables -A ipblock -s $source -j DROP");
	} else {
	    &update_ipblock_file ("$source,off,DROP,on,\<a href=\"javascript:displayGAR_Tracker('$source');\"\>BLOCKED by Guardian\</a\>,on");
	    &write_log (5,"Adding iptables DROP rule.");
	    system ("/sbin/iptables -A ipblock -s $source -j DROP");
	}
	...
	if ($block_hash{$source} <= $kicktime) {
	    ...
	    $block_hash{$source} = $kicktime;
	    &write_log (3,"$logmsg - Updated till $ktimestr");
	    ...
	    # since we're using the existing expire time, we don't need to update the hash so just carry on
	}
    }
...
sub load_saved {
 if ( -e $saved_ip_block_file_name) {
   &write_log (0,"Loading saved blocked IP data...");
   my $line;
   my $counter = 0;
   ...
   foreach $line (@current) {
     ...
     if ($block_hash{$t_line[0]} eq "") {   # do we already have an entry for this IP?
       ...
       if ($t_line[1] eq 'on') {            # does this entry have forced logging turned on?
         system ("/sbin/iptables -A ipblock -s $source -j LOG");  # add the ip to the LOG table
       }
   ...
   &write_log (0,"Loaded $counter saved IP records...");
 }
}
...
sub call_unblock {
    ...
    if ($GUI_unblock eq "FALSE") {
		...
		if ($temp[5] eq 'on') {      # "Guardian Added" flag is 'on' and source matches, so remove entries
		    ...
		    &write_log (5,"Removing iptables ipblock DROP rule");
			...
			&write_log (5,"Removing iptables ipblock LOG rule");
			...
			&write_log (5,"Removing tracker file: $source");
			if (-e $ip_tracker_file_name) {    # does the tracking file exist?
			    unlink $ip_tracker_file_name;
			} else {
			    &write_log (2,"Tracker file $source does not exist.");
			}
		    } else {
			&write_log (5,"Keeping tracker file: $source");
		    }
	...
	delete ($block_hash{$source});
	if ($killtrackfile eq "TRUE") {          # should we delete the tracking file?  TRUE: normal operation, FALSE: shutdown
	    &write_log (5,"Removing tracker file: $ip_tracker_file_name");
	    if (-e $ip_tracker_file_name) {    # does the tracking file exist?
		unlink $ip_tracker_file_name;
	    } else {
		&write_log (2,"Tracker file $ip_tracker_file_name does not exist.");
	    }
	} else {
	    &write_log (5,"Keeping tracker file: $ip_tracker_file_name");
	}
	&build_manual_hashes ("Rebuilding manual ipblock IPs hash and CIDR netblocks array...");
    }
}
...
sub clean_up_and_exit {
	...
	&write_log (0,"Saving current blocked IP data...");
	open (FILE, "$ip_block_file_name") or die 'CLEAN_UP_AND_EXIT: Unable to open ipblock config file for reading.';
	my @current = <FILE>;
	close (FILE);
	my $line;
	open (FILE, ">$saved_ip_block_file_name") or die 'CLEAN_UP_AND_EXIT: Unable to open ipblock save file for writing.';
	flock FILE, 2;
	my $counter = 0;
	foreach $line (@current) {
	    chomp($line);
