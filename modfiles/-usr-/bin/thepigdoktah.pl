...
my ($dumpvals,$filepath,$graphtime,$graphspan,$graphtimeperiod);
my ($mbps_ymax,$syns_ymax);

## let's get the current time so we know when the output was generated
my ($rtime,$ryear,$rsec,$rmin,$rhour,$rdom,$rmon,$ryoff,$rdow,$rdoy,$rdls);
($rsec,$rmin,$rhour,$rdom,$rmon,$ryoff,$rdow,$rdoy,$rdls) = localtime();
$ryear = 1900 + $ryoff;
$rmon += 1;
if (length($rmon) == 1) { $rmon = '0'.$rmon; }
if (length($rdom) == 1) { $rdom = '0'.$rdom; }
if (length($rhour) == 1) { $rhour = '0'.$rhour; }
if (length($rmin) == 1) { $rmin = '0'.$rmin; }
if (length($rsec) == 1) { $rsec = '0'.$rsec; }
$rtime = "$ryear-$rmon-$rdom $rhour:$rmin:$rsec";
...
if ($graphtime) {
  $graphtime *= 60;             # convert to seconds for time() math below
  $graphspan = int($graphtime/60/60/24);                  # days?
  if ($graphspan > 0) {
    if (length($graphspan) == 1) { $graphspan = '0'.$graphspan; }
    $graphtimeperiod = '_'.$graphspan.'days';
  } else {
    $graphspan = int($graphtime/60/60);                   # hours?
    if ($graphspan > 0) {
      if (length($graphspan) == 1) { $graphspan = '0'.$graphspan; }
      $graphtimeperiod = '_'.$graphspan.'hours';
    } else {
      $graphspan = int($graphtime/60) if $graphspan == 0; # minutes?
      if ($graphspan > 0) {
        if (length($graphspan) == 1) { $graphspan = '0'.$graphspan; }
        $graphtimeperiod = '_'.$graphspan.'mins';
      }
    }
  }
  $graphtime = (time() - $graphtime);
} else {
  $graphtime = 1; # start at the beginning of time
  $graphtimeperiod = "_";
}
...
# set mbps graph's ymax to be one more than the int(max) of mbps or packet loss
$mbps_ymax = ($stats{'mbits_wire'}{'high'}, $stats{'pkts_dropped_percentage'}{'high'}) [$stats{'mbits_wire'}{'high'} < $stats{'pkts_dropped_percentage'}{'high'}];
$mbps_ymax = int($mbps_ymax) + 1;

# set syns graph ymax to be one more than the int(max) of syns or synacks
$syns_ymax = ($stats{'syns'}{'high'}, $stats{'synacks'}{'high'}) [$stats{'syns'}{'high'} < $stats{'synacks'}{'high'}];
$syns_ymax = int($syns_ymax) + 1;
...
if ($htmlfile) {
        ...
        $htmlclose = "</pre><br><p><img src=mbps$graphtimeperiod.png></p><hr><p><img src=syns$graphtimeperiod.png></p><br> ";
} else {
        $htmlopen = "";
        $htmlclose = "";
}
...
Report Info:
        Execution Time : $rtime
        File Processed : $pmfile
        First Entry    : $min
        Last Entry     : $max
        Time Span      : $seconds
Wirespeed:
...
if ($htmlfile) {
        ...
        graph_area(\@mbpsdata,'Mbps vs Packet Loss', 'Mbps,Packet Loss', $filepath."mbps$graphtimeperiod", $mbps_ymax);
        graph_area(\@syndata,'SYNS vs SYNACKS (per second)', 'Syns/Sec,Synacks/Sec', $filepath."syns$graphtimeperiod", $syns_ymax);
...
if ($pdfile) {
        ...
        graph_area(\@mbpsdata,'Mbps vs Packet Loss', 'Mbps,Packet Loss', 'mbps', $mbps_ymax);
        graph_area(\@syndata,'SYNS vs SYNACKS (per second)', 'Syns/Sec,Synacks/Sec', 'syns', $syns_ymax);
